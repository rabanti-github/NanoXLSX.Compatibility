...
namespace NanoXLSX.Compatibility.Test.Reader
{
    ...
    {
        [Fact(DisplayName = "Test of the null reference (non-usage) of the InlinePluginHandler (for coverage)")]
        public void HandlerNullCheckTest()
        {
            ...
            using (MemoryStream stream = new MemoryStream(Encoding.UTF8.GetBytes("<xml />")))
            {
                ...
                Assert.Null(reader.InlinePluginHandler);
            }
        }
        ...
        [Fact(DisplayName = "Test of the failing Execute method on invalid XML")]
        public void ExternalLinkWorkbookReaderFailTest()
        {
            ...
            using (MemoryStream stream = new MemoryStream(Encoding.UTF8.GetBytes(xml)))
            {
                ExternalLinkWorkbookReader reader = new ExternalLinkWorkbookReader();
                reader.Init(stream, workbook, null);
                Assert.Throws<IOException>(() => reader.Execute());
            }
        }
        ...
        public void RejectsDefinedNameIdWithoutMatchingExternalReference()
        {
            ...
            Assert.Throws<IOException>(() => Execute(workbook));
            Assert.Equal("'[2]Data'!A1", workbook.GetDefinedName("ExternalTotal").TextValue);
        }

        [Fact(DisplayName = "Test no-op processing when external reference metadata is unavailable")]
        public void ReturnsWhenExternalReferenceMetadataIsUnavailable()
        {
            Workbook workbook = new Workbook("Local");
            DefinedName definedName = workbook.AddDefinedNameFormula("LocalFormula", "SUM(A1:A2)");

            Execute(workbook);

            Assert.Same(definedName, workbook.GetDefinedName("LocalFormula"));
            Assert.Equal("SUM(A1:A2)", definedName.TextValue);
        }

        [Fact(DisplayName = "Test rejection of workbook relationships without a matching external link")]
        public void RejectsWorkbookRelationshipWithoutMatchingExternalLink()
        {
            Workbook workbook = new Workbook("Local");
            AddExternalLinkData(workbook, new[] { "rIdMissing" });

            Assert.Throws<IOException>(() => Execute(workbook));
        }

        [Fact(DisplayName = "Test fallback to the finalized defined name expression when raw metadata does not match")]
        public void UsesFinalizedExpressionWhenRawDefinedNameDoesNotMatch()
        {
            Workbook workbook = new Workbook("Local");
            workbook.AddDefinedNameFormula("ExternalTotal", "'[1]Data'!A1");
            workbook.AuxiliaryData.SetData(
                PlugInUUID.CompatibilityInlineProcessor,
                CompatibilityConstants.EXTERNAL_REFERENCE_DEFINED_NAMES_ENTITY,
                new List<ExternalDefinedNameReference>
                {
                    new ExternalDefinedNameReference("DifferentName", null, "'[1]Other'!A1")
                });
            AddExternalLinkData(workbook,
                new[] { "rIdExternal1" },
                CreateExternalLink(@"C:\data\source.xlsx", "rIdExternal1"));
            ...
            Assert.Equal("'[C:\\data\\source.xlsx]Data'!A1", workbook.GetDefinedName("ExternalTotal").TextValue);
        }

        [Fact(DisplayName = "Test reconstruction of a locally scoped external name while preserving unaffected names")]
        public void RebuildsLocalExternalNameAndPreservesUnaffectedNames()
        {
            Workbook workbook = new Workbook();
            Worksheet first = new Worksheet("First");
            Worksheet second = new Worksheet("Second");
            workbook.AddWorksheet(first);
            workbook.AddWorksheet(second);
            DefinedName unaffected = workbook.AddDefinedNameConstant("Unaffected", 42);
            DefinedName external = DefinedName.ResolveDefinedName(
                "ExternalCell",
                "'[1]Data'!$A$1",
                workbook,
                second,
                "Scoped external reference");
            workbook.AddDefinedName(external);
            workbook.AuxiliaryData.SetData(
                PlugInUUID.CompatibilityInlineProcessor,
                CompatibilityConstants.EXTERNAL_REFERENCE_DEFINED_NAMES_ENTITY,
                new List<ExternalDefinedNameReference>
                {
                    new ExternalDefinedNameReference("ExternalCell", 1, "'[1]Data'!$A$1")
                });
            AddExternalLinkData(workbook,
                new[] { "rIdExternal1" },
                CreateExternalLink(@"C:\data\source.xlsx", "rIdExternal1"));
            ...
            Assert.Equal(2, workbook.GetDefinedNames().Count);
            Assert.Same(unaffected, workbook.GetDefinedNames()[0]);
            DefinedName replacement = workbook.GetDefinedName("ExternalCell", second);
            Assert.NotSame(external, replacement);
            Assert.Same(second, replacement.LocalSheet);
            Assert.Equal("'[C:\\data\\source.xlsx]Data'!$A$1", replacement.TextValue);
        }

        [Fact(DisplayName = "Test replacement of external IDs in worksheet formulas and defined name references")]
        public void UpdatesWorksheetFormulasAndDefinedNameReferences()
        {
            Workbook workbook = new Workbook("Local");
            DefinedName externalName = workbook.AddDefinedNameFormula("ExternalTotal", "'[1]Data'!A1");
            workbook.CurrentWorksheet.AddCellFormula("'[1]Data'!A1", "A1");
            workbook.CurrentWorksheet.AddCellFormula(externalName.Name, "C1");
            Cell referenceCell = workbook.CurrentWorksheet.GetCell(new Address("C1"));
            referenceCell.Formula.DefinedNameReference = externalName;
            AddExternalLinkData(workbook,
                new[] { "rIdExternal1" },
                CreateExternalLink(@"C:\data\source.xlsx", "rIdExternal1"));
            ...
            Cell regularFormula = workbook.CurrentWorksheet.GetCell(new Address("A1"));
            Assert.Equal("'[C:\\data\\source.xlsx]Data'!A1", regularFormula.Value);
            Assert.Equal("'[C:\\data\\source.xlsx]Data'!A1", regularFormula.Formula.Expression);
            Assert.Same(workbook.GetDefinedName("ExternalTotal"), referenceCell.Formula.DefinedNameReference);
        }
